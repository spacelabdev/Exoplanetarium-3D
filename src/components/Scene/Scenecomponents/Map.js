...
function Map({ data }) {
    ...
    useEffect(() => {
      ...
      for(let d = 0; d < numPlanets; ++d){
        ++count;
        console.log(data[d]);
        const x = data[d].distance * Math.cos(data[d].declination) * Math.cos(data[d].rightAscension);
        const y = data[d].distance * Math.cos(data[d].declination) * Math.sin(data[d].rightAscension);
