...
const System = ({
  ...
  useFrame(
    (state, delta) => {
      ...
      if (
        autoPositionActive &&
        Math.abs(state.camera.position.x - destinationCameraPosition.x) >
          0.01 &&
        Math.abs(state.camera.position.y - destinationCameraPosition.y) >
          0.01 &&
        Math.abs(state.camera.position.z - destinationCameraPosition.z) > 0.01
      ) {
